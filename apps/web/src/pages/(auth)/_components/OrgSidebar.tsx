import { cn } from "@/lib/utils";
import { useParams, useNavigate } from "@/router";
import { useLocation } from "react-router-dom";
import AddOrgModal from "./AddOrgModal";
import {
  TooltipContent,
  TooltipProvider,
  Tooltip,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Org } from "@/apis/orgs";

interface OrgSidebarProps {
  orgs: Org[];
  totalIncommingFriendsRequest: number;
}

export default function OrgSidebar({
  orgs = [],
  totalIncommingFriendsRequest,
}: OrgSidebarProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const { orgID } = useParams("/channels/:orgID/:channelID");
  return (
    <div className="p-4 space-y-4">
      <div
        onClick={() => navigate("/channels/@me")}
        className="p-3 bg-primary flex items-center justify-center rounded-lg cursor-pointer"
      >
        <div
          className={cn({
            "fixed left-0 bg-white h-14 rounded-r w-1":
              location.pathname === "/channels/@me",
          })}
        />
        <svg
          aria-hidden="true"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          width="30"
          height="30"
          fill="none"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
            d="M19.73 4.87a18.2 18.2 0 0 0-4.6-1.44c-.21.4-.4.8-.58 1.21-1.69-.25-3.4-.25-5.1 0-.18-.41-.37-.82-.59-1.2-1.6.27-3.14.75-4.6 1.43A19.04 19.04 0 0 0 .96 17.7a18.43 18.43 0 0 0 5.63 2.87c.46-.62.86-1.28 1.2-1.98-.65-.25-1.29-.55-1.9-.92.17-.12.32-.24.47-.37 3.58 1.7 7.7 1.7 11.28 0l.46.37c-.6.36-1.25.67-1.9.92.35.7.75 1.35 1.2 1.98 2.03-.63 3.94-1.6 5.64-2.87.47-4.87-.78-9.09-3.3-12.83ZM8.3 15.12c-1.1 0-2-1.02-2-2.27 0-1.24.88-2.26 2-2.26s2.02 1.02 2 2.26c0 1.25-.89 2.27-2 2.27Zm7.4 0c-1.1 0-2-1.02-2-2.27 0-1.24.88-2.26 2-2.26s2.02 1.02 2 2.26c0 1.25-.88 2.27-2 2.27Z"
          ></path>
        </svg>
        <h1> {totalIncommingFriendsRequest}</h1>
      </div>
      <hr />
      {orgs?.map((org) => (
        <TooltipProvider key={org.id}>
          <Tooltip>
            <TooltipTrigger className="block">
              <div
                className={cn({
                  "fixed left-0 bg-white h-14 rounded-r w-1": org.id === orgID,
                })}
              />

              <Avatar
                className="w-14 h-14"
                onClick={() =>
                  navigate("/channels/:orgID/:channelID", {
                    params: { orgID: org.id, channelID: "1" },
                  })
                }
              >
                <AvatarImage src={org.icon} alt={org.name} />
                <AvatarFallback>{org.name.slice(0, 2)}</AvatarFallback>
              </Avatar>
            </TooltipTrigger>
            <TooltipContent side="right" align="start">
              {org.name}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ))}
      <AddOrgModal />
    </div>
  );
}
